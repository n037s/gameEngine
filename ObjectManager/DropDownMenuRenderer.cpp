...
void DropDownMenuRenderer::update()
{
	...
	{
		auto objSurface = object->getSurface();

		SDL_Rect surfaceShape = rect2D(0, 0, objSurface->w, objSurface->h).tointSDL();
		SDL_Rect rel_pos = rect2D(0, i * m_itemSize.h, m_itemSize.w, m_itemSize.h).tointSDL();
